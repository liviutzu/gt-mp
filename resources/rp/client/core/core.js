...
API.onUpdate.connect(function () {
    ...
    if (API.isPlayerDead(API.getLocalPlayer())) return;
    if (global_mancare == null) return;
    var reso = API.getScreenResolution();

    if (reso.Width == 800 && reso.Height == 600) {
        API.dxDrawTexture("client/cef/img/burger.png", new Point(Math.round((resX) - 240), Math.round(resY) - 500), new Size(40, 40));
        API.drawText(global_mancare.toString() + "%", resX - 200, resY - 495, 0.5, 255, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/coffee.png", new Point(Math.round((resX) - 240), Math.round(resY) - 450), new Size(40, 40));
        API.drawText(global_apa.toString() + "%", resX - 200, resY - 445, 0.5, 255, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/notes.png", new Point(Math.round((resX) - 240), Math.round(resY) - 400), new Size(40, 40));
        API.drawText(global_bani.toString() + "$", resX - 200, resY - 395, 0.5, 114, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/credit-card.png", new Point(Math.round((resX) - 240), Math.round(resY) - 350), new Size(40, 40));
        API.drawText(global_card.toString() + "$", resX - 200, resY - 345, 0.5, 114, 219, 255, 219, 7, 0, false, true, 0);
    }
    else if (reso.Width > 800 && reso.Height > 600){
        API.dxDrawTexture("client/cef/img/burger.png", new Point(Math.round((resX) - 330), Math.round(resY) - 500), new Size(40, 40));
        API.drawText(global_mancare.toString() + "%", resX - 290, resY - 495, 0.5, 255, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/coffee.png", new Point(Math.round((resX) - 330), Math.round(resY) - 450), new Size(40, 40));
        API.drawText(global_apa.toString() + "%", resX - 290, resY - 445, 0.5, 255, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/notes.png", new Point(Math.round((resX) - 330), Math.round(resY) - 400), new Size(40, 40));
        API.drawText(global_bani.toString() + "$", resX - 290, resY - 395, 0.5, 114, 204, 114, 255, 7, 0, false, true, 0);

        API.dxDrawTexture("client/cef/img/credit-card.png", new Point(Math.round((resX) - 330), Math.round(resY) - 350), new Size(40, 40));
        API.drawText(global_card.toString() + "$", resX - 290, resY - 345, 0.5, 114, 219, 255, 219, 7, 0, false, true, 0);
    }
});
...
API.onKeyDown.connect(function (sender, e) {
    if (e.KeyCode === Keys.I && !API.isChatOpen()) {
        API.triggerServerEvent("Masina_StatusMotor");
    }
...
API.onServerEventTrigger.connect(function (eventName, args) {
    if (eventName == "CoreJS_Update_Bani")
    {
        global_bani = args[0];
    }
    if (eventName == "CoreJS_Update_Banca") {
        global_card = args[0];
    }
    if (eventName == "CoreJS_Update_Mancare") {
        global_mancare = args[0];
    }
    if (eventName == "CoreJS_Update_Apa") {
        global_apa = args[0];
    }
    ...
    if (eventName == "Inventory_arme") {
        ...
        let arunca = API.createMenuItem("Arunca", "Arunca arma.");
        ...
        ofera.Activated.connect(function (inventory_arme, item) {
            ...
            if (aruncabila == "Da") {
                ...
                API.triggerServerEvent("inventory_transferitem", numeitem, cantitateitem, cantitate, tipitem);
            }
            inventory_arme.Visible = false;
        });
    ...
    if (eventName == "Inventory_munitie") {
        player_inventory.Visible = false;
        var numeitem = args[0];
        var cantitateitem = args[1];
        var aruncabila = args[2].toString();
        var tipitem = args[3].toString();
        Inventory_munitie = API.createMenu(numeitem + " (" + cantitateitem.toString() + ") ", "", 0, 0, 6);
        let echipeaza = API.createMenuItem("Incarca", "Incarca arma a carei gloante apartine.");
        let arunca = API.createMenuItem("Arunca", "Arunca munitia.");
        let ofera = API.createMenuItem("Ofera cuiva", "Introdu cantitate de iteme pentru transfer.");
        let inchide = API.createMenuItem("Inapoi", "Inapoi la meniu.");


        echipeaza.Activated.connect(function (Inventory_munitie, item) {
            var cantitate = API.getUserInput("", 20);
            API.triggerServerEvent("Inventory_munitie_equip", numeitem, cantitate);
            Inventory_munitie.Visible = false;
        });

        arunca.Activated.connect(function (Inventory_munitie, item) {
            var cantitate = API.getUserInput("", 4);
            Inventory_munitie.Visible = false;
            API.triggerServerEvent("Inventory_arme_drop", numeitem, cantitate, cantitateitem);
        });

        ofera.Activated.connect(function (Inventory_munitie, item) {
            var cantitate = API.getUserInput("", 4);
            API.triggerServerEvent("inventory_transferitem", numeitem, cantitateitem, cantitate, tipitem);
            Inventory_munitie.Visible = false;
        });

        inchide.Activated.connect(function (Inventory_munitie, item) {
            Inventory_munitie.Visible = false;
            player_inventory.Visible = true;
        });

        Inventory_munitie.AddItem(echipeaza);
        Inventory_munitie.AddItem(arunca);
        Inventory_munitie.AddItem(ofera);
        Inventory_munitie.AddItem(inchide);
        Inventory_munitie.Visible = true;
    }
