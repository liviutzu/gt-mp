...
namespace ServerGTMP
{
    class inventorysystem : Script
    {
        ...
        public void OnClientEvent(Client player, string eventName, params object[] arguments)
        {
            ...
            if (eventName == "inventory_transferitem")
            {
                if (arguments[2].ToString().Length > 0)
                {
                    ...
                    // API.sendChatMessageToPlayer(player, "{De rezolvat} Disponibil intr-o versiune mai noua");
        ...
        private void Transfer_Items_Accept(Client player, string numejucator, int cantitate, string numeitem, string tipitem)
        {
            ...
            int Inventory_Slot_CarePoateFiAdunat = 0;
            bool Inventory_NuAre = false;
            bool Inventory_Are_PoateFiAdunatPeSlot = false;
            API.sendChatMessageToAll("ds");
            ...
            if (mai_este_jucatorul_inzona == true)
            {
                ...
                int capacitate_inventar_tinta = targetpl.getData("inventorysize");
                for (int i = 0; i < capacitate_inventar_tinta; i++)
                {
                    int cal = i + 1;
                    if (numeitem != targetpl.getData("slot" + cal.ToString()) && targetpl.getData("slot" + cal.ToString()) >= currentmaxsize)
                    {
                        Inventory_NuAre = true;
                    }
                }
                if (Inventory_NuAre == false)
                {
                    string Slot_Care_Permite_Adunarea = "";
                    string slotsender = "";
                    bool permiteadunarea = false;
                    List<int> colectie = null;
                    int rezultat = 0;

                    for (int i = 0; i < capacitate_inventar_tinta; i++)
                    {
                        int cal = i + 1; //calculul meu specific de bolanjiu
                        if (numeitem == targetpl.getData("slot" + cal.ToString()))
                        {
                            if (targetpl.getData("slot" + cal.ToString()) != null && cantitate + targetpl.getData("slot" + cal.ToString()) <= currentmaxsize)
                            {
                                Slot_Care_Permite_Adunarea = "Slot" + cal.ToString();
                                permiteadunarea = true;
                                break;
                            }
                        }
                    }
                    if (permiteadunarea == true)
                    {
                        for (int z = 0; z < player.getData("inventorysize"); z++)
                        {
                            int ca = z + 1;
                        ...
                        targetpl.setData(Slot_Care_Permite_Adunarea, targetpl.getData(Slot_Care_Permite_Adunarea) + cantitate);
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < capacitate_inventar_tinta; i++)
                        {
                            ...
                            for (int z = 0; z < colectie.Count; z++)
                            {
                                rezultat += targetpl.getData("slot" + colectie[z]);
                            }
                        }
                        if (rezultat >= cantitate)
                        {
                            for (int k = 0; k < player.getData("inventorysize"); k++)
                            {
                                int ca = k + 1;
                                if (player.getData("slot" + ca.ToString()) == numeitem && player.getData("slot" + ca.ToString() + "cont") >= cantitate)
                                {
                            ...
                            API.sendNotificationToPlayer(targetpl, "Ai primit " + cantitate.ToString() + " " + numeitem + " de la " + player.name);
                            API.sendNotificationToPlayer(player, "I-ai trimis " + cantitate.ToString() + " " + numeitem + " lui " + targetpl.name);
                            for (int y = 0; y < cantitate; y++)
                            {
                                if (cantitate == 0) break;
                                targetpl.setData("slot" + colectie[y], targetpl.getData("slot" + colectie[y] + 1));
                                player.setData(slotsender, player.getData(slotsender) - 1);
                                cantitate -= 1;
                            }
                            gamemode.PlayerUpdate(player, "pInventory");
                            gamemode.PlayerUpdate(targetpl, "pInventory");
                        ...
                        {
                            API.sendChatMessageToPlayer(player, "~r~[Info]Jucatorul nu are loc in inventar.");
                        }
                    }
                }

                if (Inventory_NuAre == true)
                {
                    bool areloc = false;
                    string slot = "";
                    string slotsender = "";
        ...
        private void inventory_echipare_munitie(Client player, string numeglont, int cantitate)
        {
            ...
            if (inventory_arearma == true) // daca are în mână să poată scoate munitia
            {
                ...
                if (nu_are_destule_iteme == false)
                {
                    ...
                    API.setPlayerWeaponAmmo(player, weapon, API.getPlayerWeaponAmmo(player, weapon) + cantitate);
        ...
        private void inventory_transfer_iteme(Client player, string numeitem, int cantitate, int cantitatedetrimis, string tipitem)
        {
            ...
            {
                if (playerList.Count > 1) //verifice daca lista de jucatori e mai mare ca 1(deoarece 1 este jucatorul local)
                {
