...
API.onServerEventTrigger.connect(function (eventName, args) {
    ...
    if (eventName == "inventory_transferitem") {
        ...
        transfera.Activated.connect(function (inventory_senditem, item) {
            API.triggerServerEvent("inventory_transferitem_accept", jucator.toString(), trimite.toString(), numeitem.toString());
    ...
    if (eventName == "Inventory_arme") {
        ...
        echipeaza.Activated.connect(function (inventory_arme, item) {
            if (aruncabila == "Da") {
                ...
                if(descos.length > 0)
                    API.triggerServerEvent("Inventory_arme_equip_aruncabila", numeitem, cantitateitem, descos);
                else
                    API.sendChatMessage("~r~[Info]Introdu o suma corecta.")

            }
            else {
                API.triggerServerEvent("Inventory_arme_equip", numeitem, cantitateitem);
            }
            inventory_arme.Visible = false;
        });
