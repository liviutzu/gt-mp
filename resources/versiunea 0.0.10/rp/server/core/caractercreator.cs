...
namespace ServerGTMP
{
    ...
    public class caractercreator : Script
    {
        public string SAVE_DIRECTORY = "Caractere";
        ...
        public void FinalStep(Client player)
        {
            API.triggerClientEvent(player, "CreateMenu");
           
        }

        public static void SendToWorld(Client player)
        {
            string nume = API.shared.getEntityData(player, "nume");
            string prenume = API.shared.getEntityData(player, "prenume");
            string fullname = nume + " " + prenume;
            player.dimension = 0;

            API.shared.setEntityData(player, "PlayerFullName", fullname);

            databaseMaster.isSpawned = true;
