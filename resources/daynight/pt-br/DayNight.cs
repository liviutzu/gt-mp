...
namespace TimeScript
{
    ...
    {
        string[] dayNames = { "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sabado", "Domingo" };
        ...
        public void CMDWorldTime(Client sender, int hour, int minute)
        {
            ...
            {
                API.sendChatMessageToPlayer(sender, "~r~ERRO: ~w~Você não tem permissão para usar esse comando.");
                return;
            }

            if (hour < 0 || hour > 23)
            {
                API.sendChatMessageToPlayer(sender, "~r~ERRO: ~w~Hora inválida.");
                return;
            }

            if (minute < 0 || minute > 59)
            {
                API.sendChatMessageToPlayer(sender, "~r~ERRO: ~w~Minuto inválido.");
                return;
            }
        ...
        public void CMDWorldDay(Client sender, int day)
        {
            ...
            {
                API.sendChatMessageToPlayer(sender, "~r~ERRO: ~w~Dia inválido.");
                return;
            }
