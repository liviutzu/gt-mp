...
namespace ServerGTMP
{
    ...
    public class caractercreator : Script
    {
        ...
        public void SendToWorld(Client player)
        {
            string nume = API.getEntityData(player, "nume");
            string prenume = API.getEntityData(player, "prenume");
            string fullname = nume + " " + prenume;
            player.dimension = 0;

            API.setEntityData(player, "PlayerFullName", fullname);
